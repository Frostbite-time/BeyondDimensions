...
public class XpExchangeMenu extends BDBaseMenu
{
    ...
    private static final int INV_SLOT_START_Y = CommonTextures.TOP_BASE_COMMON_HEIGHT + CommonTextures.COMMON_CONNECTION_HEIGHT * 5 + 7;
    ...
    private void addPlayerInv(Inventory playerInventory)
    {
        ...
        for (int row = 0; row < 3; ++row)
        {
            ...
            {
                this.addSlot(new Slot(playerInventory, col + row * 9 + 9, 8 + col * 18, INV_SLOT_START_Y + row * 18));
            }
        }
        for (int col = 0; col < 9; ++col)
        {
            this.addSlot(new Slot(playerInventory, col, 8 + col * 18, 4 + INV_SLOT_START_Y + 3 * 18));
        }
    ...
    protected boolean shouldSendQuickData()
    {
        ...
        boolean shouldSend = super.shouldSendQuickData()
    ...
    public void readQuickDataTag(CompoundTag tag)
    {
        ...
        if (tag.contains("xp_keep_mode"))
            menuStack.set(BDDataComponents.XP_NET_KEEP_MODE, tag.getBoolean("xp_keep_mode").orElse(false));

        if (tag.contains("xp_target_level"))
            XpExchangeSettings.setTargetLevel(menuStack, tag.getInt("xp_target_level").orElse(XpExchangeSettings.DEFAULT_TARGET_LEVEL));
    }
}
