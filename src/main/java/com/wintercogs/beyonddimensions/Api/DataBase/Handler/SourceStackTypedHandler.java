...
public class SourceStackTypedHandler implements ISourceCap
{
    ...
    // 获取当前魔源量(仅获取第一个读取到的魔源，当抽干之后会继续读取下一个)
    @Override
    ...
    {
        return Integer.MAX_VALUE; // 让外界认为可以无限存储，实际由接收和导出方法说了算，以便于控制
    }
    ...
    {
        int wanted = BDMath.clampLongToInt(handlerStorage.getSlotCapacity(0));
