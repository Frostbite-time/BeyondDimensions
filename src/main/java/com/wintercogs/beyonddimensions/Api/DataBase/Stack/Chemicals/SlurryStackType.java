...
public final class SlurryStackType implements IStackType<SlurryStack>
{
    ...
    {
        stack = SlurryStack.EMPTY;
    }

    // 创建给定stack
    public SlurryStackType(SlurryStack stack)
    {
        this.stack = stack;
    }
    ...
    public IStackType<SlurryStack> fromObject(Object key, long amount, CompoundTag dataComponentPatch)
    {
        ...
        {
            SlurryStack chemicalStack = new SlurryStack(chemical, amount);
            return new SlurryStackType(chemicalStack);
        }
        return null;
    }
    ...
    {
        return new SlurryStack(stack, count);
    }
    ...
    public IStackType<SlurryStack> split(long amount)
    {
        ...
        split.setAmount(splitAmount);
        stack.shrink(splitAmount);
        return new SlurryStackType(split);
    }
    ...
    public boolean hasTag(TagKey<?> tagKey)
    {
        ...
        if (!tagKey.isFor(MekanismAPI.SLURRY_REGISTRY_NAME)) {
            return false;
        }

        TagKey<Slurry> chemicalTag = (TagKey<Slurry>) tagKey;
    ...
    public void serialize(FriendlyByteBuf buf)
    {
        ...
        if (hasItem) {
            ...
            // 使用副本避免修改原堆栈
            SlurryStack copy = new SlurryStack(stack,1);
    ...
    public IStackType<SlurryStack> deserialize(FriendlyByteBuf buf, ResourceLocation typeId)
    {
        ...
        if (!hasItem) {
            return new SlurryStackType(SlurryStack.EMPTY);
        }

        // 读取数量
        long count = buf.readVarLong();
        // 使用OPTIONAL_CODEC解码
        SlurryStack stack = new SlurryStack(SlurryStack.readFromPacket(buf),count);
        return new SlurryStackType(stack);
    }
    ...
    {
        if(other instanceof SlurryStackType otherStack)
        {
