...
public class OrderedStackTypedSlot extends AbstractStackTypedSlot
{
    ...
    public void click(IStackKey clickStack, int button, Player player)
    {
        ...
        if (clickStack.isEmpty())
        {
            ...
            {   //槽位物品为空，携带物品存在，将携带物品插入槽位
                ...
                if (!handled.get())
                {
                    ...
                    int remaining = (int) storage.insert(getSlotIndex(), StackCreater.Create(ItemStackKey.ID, carriedItem.copyWithCount(changedCount), changedCount), false).getStackAmount();
        ...
        else if (mayPickup(player))
        {
            ...
            else if (mayPlace(carriedItem))
            {
                ...
                if (clickStack.isSameTypeSameComponents(new ItemStackKey(carriedItem.copy())))
                {
                    int changedCount = button == GLFW.GLFW_MOUSE_BUTTON_LEFT ? carriedItem.getCount() : 1;
                    int remaining = (int) storage.insert(getSlotIndex(), StackCreater.Create(ItemStackKey.ID, carriedItem.copyWithCount(changedCount), changedCount), false).getStackAmount();
                    int actualInsert = changedCount - remaining; // 实际被插入的物品数量
                ...
                else  // 槽位物品存在，携带物品存在，不为相同类型
                {
                    ...
                    if (carriedItem.getItem() instanceof XpExchangeItem && button != GLFW.GLFW_MOUSE_BUTTON_LEFT)
                    {
                        IStackKey actualStack = getStack();
                        ...
                        if (actualStack instanceof FluidStackKey fluidStackKey && fluidStackKey.hasTag(ModFluidTags.C_EXPERIENCE))
                        {
                            ...
                            else if (button == GLFW.GLFW_MOUSE_BUTTON_MIDDLE) // 鼠标中键--取出一级
                            {
                                ...
                                // 首先尝试提取指定数量的经验流体
                                IStackKey extracted = storage.extract(getSlotIndex(), actualRemoveFluid, false);
                    ...
                    {
                        IStackKey actualStack = getStack();
                        if (actualStack instanceof ItemStackKey)
                        {
                            if (carriedItem.getCount() <= getSlotCap() && actualStack.getStackAmount() <= actualStack.getVanillaMaxStackSize())
                            {
                                // 鼠标携带的数量，小于等于槽位容量
                                // 槽位当前物品数量，小于等于其原版最大数量
                                ItemStackKey extract = (ItemStackKey) storage.extract(getSlotIndex(), actualStack.getStackAmount(), false);
                                IStackKey remaining = storage.insert(getSlotIndex(), new ItemStackKey(carriedItem), true);
                                if (remaining.isEmpty())
                                {
                                    // 全部插入时则完成交换
                                    storage.insert(getSlotIndex(), new ItemStackKey(carriedItem), false);
                            ...
                            CapabilityHelper.ItemCapabilityMap.forEach((typeId, cap) -> {
                                ...
                                if (clickStack.getTypeId().equals(typeId))
                                {
                                    ...
                                    if (handler.isPresent())
                                    {
                                        ...
                                        {
                                            // 获取真实最大值 防止数据包伪造
                                            IStackKey trueStack = storage.getStackBySlot(getSlotIndex());
                                            long tureCount = 0;
    ...
    public void quickMove(IStackKey clickStack, int button, Player player)
    {
        ...
        if (!clickStack.isEmpty())
        {
            ...
            IStackKey trueStack = storage.getStackBySlot(theSlot).copyWithCount(clickStack.getStackAmount());
            ...
            for (int targetSlotIndex = quickMoveSlotStartIndex; targetSlotIndex < quickMoveSlotEndIndex && !trueStack.isEmpty(); targetSlotIndex++)
            {
                ...
                if (slot instanceof AbstractStackTypedSlot aSlot)
                {
                    ...
                    //首先尝试从存储提取指定堆叠
                    IStackKey extract = safeExtract(trueStack);
                ...
                else
                {
                    ...
                    if (trueStack instanceof ItemStackKey trueItemTypedStack)
                    {
                        ...
                        if (!remaining.isEmpty())
                            safeInsert(new ItemStackKey(remaining));
                    ...
                    else if (trueStack instanceof FluidStackKey trueFluidTypedKey && trueFluidTypedKey.getSource().getBucket() != Items.AIR)
                    {
                        ...
                        IStackKey<?> bucket = storage.extract(new ItemStackKey(new ItemStack(Items.BUCKET)), false);
                        if (bucket.isEmpty())
                        {
                            safeInsert(extract);
                            break;
