            ...
            {   //槽位物品为空，携带物品存在，将携带物品插入槽位
                ...
                if (carriedItem.getItem() instanceof XpExchangeItem && button != GLFW.GLFW_MOUSE_BUTTON_LEFT)
                {
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) // 鼠标右键--存入一级
                    {
                        ...
                        // 插入当前经验流体
                        IStackKey remaining = storage.insert(new FluidStackKey(new FluidStack(ModFluids.XP_FLUID.source().get(), 1), actualInsertFluid), false);
                ...
                if (!handled.get())
                {
                    ...
                    int actualInsert = (int) (changedCount - storage.insert(StackCreater.Create(ItemStackKey.ID, carriedItem.copyWithCount(changedCount), changedCount), false).getStackAmount());
        ...
        else if (mayPickup(player))
        {
            ...
            if (carriedItem.isEmpty())
            {
                if (clickStack instanceof ItemStackKey clickItem)
                {
                    ...
                    ItemStack takenItem = ((ItemStack) storage.extract(new ItemStackKey(clickItem.copyStackWithCount(actualChangeNum)), false).getStack()).copy();
                    if (takenItem != null)
                    {
                ...
                if (carriedItem.getItem() instanceof XpExchangeItem && button != GLFW.GLFW_MOUSE_BUTTON_LEFT)
                {
                    ...
                    if (actualStack instanceof FluidStackKey fluidStackKey && fluidStackKey.hasTag(ModFluidTags.C_EXPERIENCE))
                    {
                        ...
                        if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) // 鼠标右键--存入一级
                        {
                            ...
                            // 插入当前经验流体
                            IStackKey remaining = storage.insert(fluidStackKey.copyWithCount(actualInsertFluid), false);
                            if (!remaining.isEmpty())
                            {
                                int needReturnXp = BDMath.clampLongToInt(remaining.getStackAmount() / 20); // 由于前面从int*20，这里除回去
                                actualRemovePlayerXp = actualRemovePlayerXp - needReturnXp;
                        ...
                        else if (button == GLFW.GLFW_MOUSE_BUTTON_MIDDLE) // 鼠标中键--取出一级
                        {
                            ...
                            // 首先尝试提取指定数量的经验流体
                            IStackKey extracted = storage.extract(fluidStackKey.copyWithCount(actualRemoveFluid), false);
                            ...
                            if (carriedItem.getItem() == Items.BUCKET) // 空桶接受
                            {
                                if (clickStack instanceof FluidStackKey fluidStackKey)
                                {
                                    ...
                                    if (filledBucket != null && filledBucket != Items.AIR
                                            && storage.getStackByStack(fluidStackKey).getStackAmount() >= 1000)
                                    {
                                        // 执行操作
                                        storage.extract(fluidStackKey.copyWithCount(1000), false);
