...
public class XpUtil
{
    ...
    public static int clampXpToGive(Player player, long requestedXp)
    {
        ...
        return (int) Math.min(Math.min(levelRoom, totalRoom), Math.max(0L, requestedXp));
    }
    ...
    public static int xpToGiveForLevels(Player player, int levels)
    {
        ...
        return (int) Math.min((long) available, Math.max(0L, xpBetweenLevels(currentLevel, targetLevel)));
    }
