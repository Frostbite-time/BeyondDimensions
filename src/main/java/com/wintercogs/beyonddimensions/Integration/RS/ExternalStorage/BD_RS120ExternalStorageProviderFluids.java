    ...
    public IExternalStorage<FluidStack> provide(IExternalStorageContext ctx, BlockEntity externalBe, Direction direction) {
        if (!(externalBe.getLevel() instanceof ServerLevel serverLevel)) {
            ...
            return new IExternalStorage<>() {
                @Override public void update(com.refinedmods.refinedstorage.api.network.INetwork network) {}
                @Override public long getCapacity() { return 0; }
                @Override public java.util.List<FluidStack> getStacks() { return java.util.Collections.emptyList(); }
                @Override public FluidStack insert(FluidStack prototype, int size, com.refinedmods.refinedstorage.api.util.Action action) { return prototype.copy(); }
                @Override public FluidStack extract(FluidStack prototype, int size, int flags, com.refinedmods.refinedstorage.api.util.Action action) { return FluidStack.EMPTY; }
                @Override public int getStored() { return 0; }
                @Override public int getPriority() { return ctx.getPriority(); }
                @Override public com.refinedmods.refinedstorage.api.storage.AccessType getAccessType() { return ctx.getAccessType(); }
                @Override public int getCacheDelta(int storedPreInsertion, int size, FluidStack remainder) {
